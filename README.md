...
- Runs on *Worker 1*.  
- Docker image: harsha497/library:latest  
...
- name: Deploy Digital Library App on worker1
  ...
  tasks:
    ...
    - name: Pull latest image
      command: docker pull harsha497/library:latest

    - name: Run new container
      command: >
        docker run -d --name library
        -p 8082:8080
       harsha497/library:latest
...
- Runs on *Worker 2*.  
- Docker image: harsha497/snakegame:latest  
...
- name: Deploy Snake Game App on worker2
  ...
  tasks:
    ...
    - name: Pull latest image
      command: docker pull harsha497/snakegame:latest

    - name: Run new container
      command: >
        docker run -d --name snakegame
        -p 8081:80
        harsha497/snakegame:latest
